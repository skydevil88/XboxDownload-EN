    ...
    public partial class Form1 : Form
    {
        ...
        public async void ButStart_Click(object? sender, EventArgs? e)
        {
            ...
            else
            {
                ...
                string? gameIP = null;
                if (!string.IsNullOrWhiteSpace(tbGameIP.Text))
                {
                    if (IPAddress.TryParse(tbGameIP.Text.Trim(), out IPAddress? ipAddress) && ipAddress.AddressFamily == AddressFamily.InterNetwork)
                    {
                        gameIP = tbGameIP.Text = ipAddress.ToString();
                    }
                    else
                    {
        ...
        private static void UpdateHosts(bool add)
        {
        ...
        private void GameSearch(string query)
        {
            Thread.Sleep(300);
            if (this.query != query) return;
            string url = "https://www.microsoft.com/msstoreapiprod/api/autosuggest?market=en-US&clientId=7F27B536-CF6B-4C65-8638-A0F8CBDFCA65&sources=Microsoft-Terms,Iris-Products,xSearch-Products&filter=+ClientType:StoreWeb&counts=5,1,5&query=" + ClassWeb.UrlEncode(query);
                    ...
                    using (HttpResponseMessage? response = ClassWeb.HttpResponseMessage("https://" + ip + "/GetBasePackage/" + contentId, "GET", null, null, new() { { "Host", hosts }, { "Authorization", Properties.Settings.Default.Authorization } }))
                    {
                        if (response != null)
                        {
                            ...
                            else if (response.StatusCode == HttpStatusCode.Unauthorized)
                            {
                                ...
                                Properties.Settings.Default.Save();
                                if (platform == 2) url = "Authorization expired. 1.Back to Service tab Click \"Enable HTTP(S) service\" and \"Speed up MS Store (PC)\"  then press \"Start\" butten. 2.Open Xbox app, select a game and click install, await the log show out download link then complete update authorization.";
                            }
                        }
                    }
                    this.Invoke(new Action(() =>
            ...
            if (lvGame.SelectedItems.Count == 1)
            {
                ...
                else if (!string.IsNullOrEmpty(text) && !Regex.IsMatch(text, @"Please wait..."))
                {
                    if (!Regex.IsMatch(text, @"^Authorization expired"))
                    {
                        ...
                        tsmCopyUrl2.Enabled = isGame && item.SubItems[3].Tag != null && Regex.IsMatch(item.SubItems[3].Tag.ToString() ?? string.Empty, @"http://[^\.]+\.xboxlive\.com/(\d{1,2}|Z)/");
