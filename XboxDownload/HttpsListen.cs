                    ...
                    if (ssl.IsAuthenticated)
                    {
                        while (Form1.bServiceFlag && mySocket.Connected && mySocket.Poll(3000000, SelectMode.SelectRead))
                        {
                            ...
                            if (Properties.Settings.Default.LocalUpload && !string.IsNullOrEmpty(_localPath))
                            {
                                ...
                                if (fs != null)
                                {
                                    if (Properties.Settings.Default.RecordLog) parentForm.SaveLog("Local Upload", _localPath, mySocket.RemoteEndPoint != null ? ((IPEndPoint)mySocket.RemoteEndPoint).Address.ToString() : string.Empty);
                                    ...
                                    case "packagespc.xboxlive.com":
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(ip))
                                            {
                                                ...
                                                if (response != null && response.IsSuccessStatusCode)
                                                {
                                                    ...
                                                    string str = "HTTP/1.1 200 OK\r\n" + Regex.Replace(response.Content.Headers.ToString(), @"(Content-Encoding|Transfer-Encoding|Content-Length): .+\r\n", "") + "Content-Length: " + buffer.Length + "\r\n" + response.Headers;
                                                    ...
                                                    if (Regex.IsMatch(html, @"^{.+}$"))
                                                    {
                                                        ...
                                                        if (packageFiles != null)
                                                        {
                                                            ...
                                                            if (Properties.Settings.Default.RecordLog) parentForm.SaveLog("Download Link", url, mySocket.RemoteEndPoint != null ? ((IPEndPoint)mySocket.RemoteEndPoint).Address.ToString() : string.Empty, 0x008000);
