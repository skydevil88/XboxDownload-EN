                    ...
                    if (ssl.IsAuthenticated)
                    {
                        while (Form1.bServiceFlag && mySocket.Connected && mySocket.Poll(3000000, SelectMode.SelectRead))
                        {
                            ...
                            if (Properties.Settings.Default.LocalUpload && !string.IsNullOrEmpty(_localPath))
                            {
                                ...
                                if (fs != null)
                                {
                                    if (Properties.Settings.Default.RecordLog) parentForm.SaveLog("Local Upload", _localPath, ((IPEndPoint)mySocket.RemoteEndPoint!).Address.ToString());
                                                ...
                                                if (response != null && response.IsSuccessStatusCode)
                                                {
                                                    ...
                                                    if (Regex.IsMatch(html, @"^{.+}$"))
                                                    {
                                                        ...
                                                        if (packageFiles != null)
                                                        {
                                                            ...
                                                            if (Properties.Settings.Default.RecordLog) parentForm.SaveLog("Download Link", url, ((IPEndPoint)mySocket.RemoteEndPoint!).Address.ToString(), 0x008000);
                                                ...
                                                if (string.IsNullOrEmpty(socketPackage.Err))
                                                {
                                                    ...
                                                    if (Properties.Settings.Default.RecordLog)
                                                    {
                                                        ...
                                                        if (_filePath.StartsWith("/ecommerce2/downloadURL"))
                                                        {
                                                            ...
                                                            if (m1.Success) parentForm.SaveLog("Download Link", m1.Groups["url"].Value, ((IPEndPoint)mySocket.RemoteEndPoint!).Address.ToString(), 0x008000);
                                                        }
                                                    }
                                                }
                                            }
